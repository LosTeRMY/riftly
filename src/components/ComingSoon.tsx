export default function ComingSoon() {
  return (
    <main className="relative h-[calc(100vh-64px)] w-full flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-4xl px-8 flex flex-col items-center text-center">
        {/* Content card */}
        <div className="bg-surface-container-low/40 backdrop-blur-xl border-l-4 border-primary-container p-12 md:p-24 rounded-xl shadow-[0_0_80px_-20px_rgba(0,255,148,0.15)] relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            {/* Headline */}
            <h1 className="font-display text-5xl md:text-7xl lg:text-8xl font-bold text-primary tracking-tight leading-[1.1] mb-8">
              Feature coming <br />
              <span className="text-primary-container">soon.. :)</span>
            </h1>

            {/* Description */}
            <p className="font-body text-on-surface-variant text-lg md:text-xl max-w-xl mx-auto leading-relaxed mb-12">
              We're currently refining this section to deliver an unparalleled analytical experience.
              Our engineers are fine-tuning the data streams for maximum precision.
            </p>

            {/* CTA buttons */}
            <div className="flex flex-col md:flex-row items-center justify-center gap-4">
              <button className="px-8 py-4 bg-primary-container text-on-primary-container font-label font-bold rounded-lg transition-transform active:scale-95 duration-150 flex items-center gap-2 group-hover:shadow-[0_0_20px_rgba(0,255,148,0.3)]">
                <span className="material-symbols-outlined">rocket_launch</span>
                Notify Me
              </button>
              <button className="px-8 py-4 border border-outline-variant text-on-surface font-label font-bold rounded-lg hover:bg-surface-container-highest transition-all duration-300">
                Return to Dashboard
              </button>
            </div>
          </div>
        ...
        <div className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-8 opacity-40">
          {["Real-time Data", "Global Ranks", "Pro Insights", "Advanced API"].map((label) => (
